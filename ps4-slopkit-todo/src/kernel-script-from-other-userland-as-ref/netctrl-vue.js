...
var KQUEUE_ITERATIONS = 6500;
var MAX_ROUNDS_TWIN = 6;
...
function init() {
  log('=== PS4 NetControl Jailbreak ===');
  log('build: eb8d306-staging');
  log('tag: 2.0');
...
function yield_to_render(callback) {
  var id = jsmaf.setInterval(function () {
    ...
    try {
      ...
      log('ERROR: ' + e.message);
	  utils.notify('Poopsploit failed! Reboot and try again');
      cleanup();
    }
  }, 0);
}
...
function exploit_phase_trigger() {
  if (exploit_count >= MAIN_LOOP_ITERATIONS) {
    ...
    cleanup();
	throw new Error('NetControl failed - Shutdown and try again');
    return;
  }
  exploit_count++;
