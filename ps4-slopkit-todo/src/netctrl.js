       ...
       */
            ...
            for (const w of workers) {
                try {
                    ...
                    await fireW(w, SYS.rtprio_thread, [RTP_SET, 0, prioAddr],
                        WORKER_ATTR_MS);
                    wr++;
                    mark("WORKER-ATTR-RESTORED", "at=" + why + " " + w.name
                        + " ok " + wr + "/" + wn);
                } catch (e) {
                    wf++;
                    mark("WORKER-ATTR-SKIPPED", "at=" + why + " " + w.name
                        + " -- " + ((e && e.message) ? e.message : String(e))
                        + " (best effort; main thread is already restored)");
                }
            }
            mark("WORKER-ATTRS-RESTORED", "at=" + why + " n=" + wr + "/" + wn
                + (wf ? " failed=" + wf : "") + " timeout_ms=" + WORKER_ATTR_MS);
        }
        ...
        */
        async function boundedJoin(tasks, ms, label) {
            ...
            if (outstanding > 0)
                mark("JOIN-TIMEOUT", label + " " + outstanding + "/" + tasks.length
                    + " parked after " + ms + " ms");
            return outstanding;
        }
        ...
        async function landUio(size, forWrite, tasks) {
            ...
            */
            ...
            for (let i = 0; i < NUM_UIO_SPRAY; ++i) {
                ...
                await boundedJoin(tasks, JOIN_MS, "land-uio");
        ...
        */
        ...
        for (const w of workers) {
            try {
                ...
                w.armed = false;
                disarmed++;
                mark("WORKER-DISARMED", w.name + " ok");
            } catch (e) {
                disarmFailed++;
                mark("DISARM-THREW", w.name + " "
                    + ((e && e.message) ? e.message : String(e))
                    + " (best effort; terminate() follows)");
            }
        }
        if (workers.length)
